...
function symbolMap() {
    ...
    function chart(id) {
        ...
        // var earthquakes = svg.append("g").attr("id", "earthquakes");
        ...
        //     .datum(topojson.mesh(map, map.objects.countries).features)
        //     .attr("class", "boundary")
        //     .attr("d", path);
            // .classed({"country": true});
    ...
    function hideHighlight(d) {
        ...
        d3.select("g#country")
            .select("path#country" + lookup[d.country])
            .classed({
                "highlight": false,
                "state": true
            });
    ...
    // function zoomed() {
    //     d3.select("g#world").attr("transform", "translate(" + d3.event.translate + ")scale(" + d3.event.scale + ")");
    // }
